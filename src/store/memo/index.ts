...
export const initailState: MemoState = {
  search: '',
...
const slice = createSlice({
  ...
  reducers: {
    ...
    deleteMemo(state, action: PayloadAction) {
      ...
      const srotedMemos = [...state.memolist].sort(
        (a, b) =>
          new Date(b.created_at).getTime() - new Date(a.created_at).getTime(),
      );

      if (srotedMemos.length > 0) {
        const memo = state.memolist.find(memo => memo.id === srotedMemos[0].id);
        if (memo) {
          memo.selected = true;
        }
      }
